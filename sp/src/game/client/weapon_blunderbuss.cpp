...
class CWeaponBlunderbuss : public CBaseHLCombatWeapon
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS( CWeaponBlunderbuss, CBaseHLCombatWeapon );
...
public:
	...
	{
		static Vector vitalAllyCone = VECTOR_CONE_10DEGREES;
		static Vector cone = VECTOR_CONE_15DEGREES;

		if( GetOwner() && (GetOwner()->Classify() == CLASS_PLAYER_ALLY_VITAL) )
		{
			// Give Alyx's blunderbuss blasts more a more directed punch. She needs
...
//	void WeaponIdle( void );
	...
	CWeaponBlunderbuss(void);
};

IMPLEMENT_SERVERCLASS_ST(CWeaponBlunderbuss, DT_WeaponBlunderbuss)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_blunderbuss, CWeaponBlunderbuss );
PRECACHE_WEAPON_REGISTER(weapon_blunderbuss);

BEGIN_DATADESC( CWeaponBlunderbuss )
...
acttable_t	CWeaponBlunderbuss::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponBlunderbuss);

void CWeaponBlunderbuss::Precache( void )
{
...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
//-----------------------------------------------------------------------------
// Purpose:	When we shipped HL2, the blunderbuss weapon did not override the
...
//			we are going to fix the combine's rate of blunderbuss fire in episodic.
//			This change will not affect Alyx using a blunderbuss in EP1. (sjb)
//-----------------------------------------------------------------------------
float CWeaponBlunderbuss::GetMinRestTime()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeaponBlunderbuss::GetMaxRestTime()
{
	//if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	//{
		return 6.0f;
...
//-----------------------------------------------------------------------------
float CWeaponBlunderbuss::GetFireRate()
{
	//if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	//{
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponBlunderbuss::StartReload( void )
{
	...
	// If blunderbuss totally emptied then a pump animation is needed
	...
	*/
	...
	// Make blunderbuss shell visible
	SetBodygroup(1,0);
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponBlunderbuss::Reload( void )
{
	// Check that StartReload was called first
	if (!m_bInReload)
	{
		Warning("ERROR: Blunderbuss Reload called incorrectly!\n");
	}
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::FinishReload( void )
{
	// Make blunderbuss shell invisible
	SetBodygroup(1,1);
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::FillClip( void )
{
	CBaseCombatCharacter *pOwner  = GetOwner();
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::Pump( void )
{
	CBaseCombatCharacter *pOwner  = GetOwner();
...
//
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::DryFire( void )
{
	WeaponSound(EMPTY);
...
//
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::PrimaryAttack( void )
{
...
//
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::SecondaryAttack( void )
{
	/*
	// Only the player fires this way so we can cast
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	...
	// Don't fire again until fire animation has completed
	m_flNextPrimaryAttack = gpGlobals->curtime + SequenceDuration();
	m_iClip1 -= 2;	// Blunderbuss uses same clip for primary and secondary attacks
...
//-----------------------------------------------------------------------------
// Purpose: Override so blunderbuss can do mulitple reloads in a row
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::ItemPostFrame( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBlunderbuss::ItemHolsterFrame( void )
{
	// Must be player held
...
//==================================================
/*
void CWeaponBlunderbuss::WeaponIdle( void )
{
	//Only the player fires this way so we can cast
